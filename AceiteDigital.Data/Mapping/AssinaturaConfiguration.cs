...
namespace AceiteDigital.Data.Mapping
{
    public class AssinaturaConfiguration : IEntityTypeConfiguration<Assinatura>
    {
        public void Configure(EntityTypeBuilder<Assinatura> builder)
        {
            ...
            builder.Property(p => p.Assinado).HasColumnName("Assinado");

            builder.Property(p => p.DataHoraRegistro).IsRequired().HasColumnName("DataHorarioRegistro").HasColumnType("datetime");

            builder.HasOne(p => p.DocumentoSignatario).WithOne(d => d.Assinatura).HasForeignKey<Assinatura>(e => e.DocumentoSignatarioId)
